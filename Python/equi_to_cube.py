...
outputImage.save(os.path.splitext("results/" + sys.argv[1])[0] + "_CUBE.png")

# Extract individual cube faces
sqr = int(sqr)  # face size (assumed square)
outputImage = Image.new("RGB", ((int(outputWidth)), (int(outputHeight))), None)
outputImage.putdata(output)

# Face crops (left, upper, right, lower)
faces = {
    "top":    outputImage.crop((0, 0, sqr, sqr)),
    "right":  outputImage.crop((sqr, 0, sqr*2, sqr)),
    "bottom": outputImage.crop((sqr*2, 0, sqr*3, sqr)),

    "left":   outputImage.crop((0, sqr, sqr, sqr*2)),
    "back":   outputImage.crop((sqr, sqr, sqr*2, sqr*2)),
    "front":  outputImage.crop((sqr*2, sqr, sqr*3, sqr*2)),
}

# Save them
base_name = os.path.splitext(sys.argv[1])[0]
for name, img in faces.items():
    img.save(f"results/{base_name}_{name.upper()}.png")

print("All cube faces saved: TOP, BOTTOM, LEFT, RIGHT, FRONT, BACK")
